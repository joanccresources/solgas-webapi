...
class ContentFooterMenuRequest extends FormRequest
{
    /**
    ...
    {
        $menuId = $this->route('content_footer_menu'); // ID del menú si es update
        $parentId = $this->input('content_footer_menu_id'); // ID del padre
        ...
        $rules = [
            'name' => [
                ...
                function ($attribute, $value, $fail) use ($menuId, $parentId) {
                    ...
                    $exists = ContentFooterMenu::where('name', $value)
                        ->where('content_footer_menu_id', $parentId) // Solo dentro del mismo nivel
            ...
            'content_footer_id' => ['bail', 'required', 'integer', Rule::exists('content_footers', 'id')],
            'content_footer_menu_id' => [
                'bail',
                'nullable',
                'integer',
                Rule::exists('content_footer_menus', 'id')->whereNot('id', $menuId?->id) // No permitir ser su propio padre
            ],
