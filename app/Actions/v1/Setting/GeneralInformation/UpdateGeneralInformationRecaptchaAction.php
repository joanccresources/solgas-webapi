<?php

declare(strict_types=1);

namespace App\Actions\v1\Setting\GeneralInformation;

use App\DTOs\v1\Setting\GeneralInformation\UpdateGeneralInformationRecaptchaDto;
use App\Http\Controllers\API\v1\Helpers\ApiResponse;
use App\Http\Resources\v1\GeneralInformationResource;
use App\Models\GeneralInformation;
use Illuminate\Http\JsonResponse;

class UpdateGeneralInformationRecaptchaAction
{
    protected $element;

    public function __construct(GeneralInformation $element)
    {
        $this->element = $element;
    }

    public function execute(UpdateGeneralInformationRecaptchaDto $dto)
    {
        return $this
            ->update($dto)
            ->buildResponse($dto);
    }

    protected function update(UpdateGeneralInformationRecaptchaDto $dto): self
    {
        $this->element->recaptcha_secret_key = $dto->recaptcha_secret_key;
        $this->element->recaptcha_site_key = $dto->recaptcha_site_key;
        $this->element->recaptcha_google_url_verify = $dto->recaptcha_google_url_verify;

        $this->element->save();

        return $this;
    }

    protected function buildResponse($dto): JsonResponse
    {
        return ApiResponse::createResponse()
            ->withData(new GeneralInformationResource($this->element))
            ->withMessage(trans('custom.message.update.success', ['name' => trans('custom.attribute.general_information')]))
            ->build();
    }
}
